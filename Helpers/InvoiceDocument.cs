...
namespace QuestPDF.ExampleInvoice
{
    public class InvoiceDocument : IDocument
    {
        ...
        public InvoiceDocument(InvoiceData model)
        {
            Model = model;
        }
        ...
        void ComposeHeader(IContainer container)
        {
            container.Row(row =>
            {
                ...
                {
                    column
                        .Item().Text($"Invoice #{Model.InvoiceInfo.Number}")
                        .FontSize(20).SemiBold().FontColor(Colors.Blue.Medium);

                    column.Item().Text(text =>
                    {
                        text.Span("Issue date: ").SemiBold();
                        text.Span($"{Model.InvoiceInfo.CreatedAt:d}");
                    });
                    
                    column.Item().Text(text =>
                    {
                        text.Span("Due date: ").SemiBold();
                        text.Span($"{Model.InvoiceInfo.PaymentDate:d}");
                    });
                });
        ...
        void ComposeContent(IContainer container)
        {
            container.PaddingVertical(40).Column(column => 
            {
                ...
                column.Item().Row(row =>
                {
                    row.RelativeItem().Component(new AddressComponent("From", Model.InvoiceInfo.GetAdress(Invoice.CompanyEnu.Seller)));
                    row.ConstantItem(50);
                    row.RelativeItem().Component(new AddressComponent("For", Model.InvoiceInfo.GetAdress(Invoice.CompanyEnu.Buyer)));
                });
        ...
        void ComposeTable(IContainer container)
        {
            ...
            container.Table(table =>
            {
                ...
                foreach (var item in Model.ProductsInfo)
                {
                    var index = Model.ProductsInfo.IndexOf(item) + 1;

                    table.Cell().Element(CellStyle).Text($"{index}");
                    table.Cell().Element(CellStyle).Text(item.Name);
                    table.Cell().Element(CellStyle).AlignRight().Text($"{item.PostDiscountNettoValue:C}");
        ...
        void ComposeComments(IContainer container)
        {
            container.ShowEntire().Background(Colors.Grey.Lighten3).Padding(10).Column(column => 
            {
                ...
                column.Item().Text("Testowy komentarz do faktury - lorem ipsum!");
            });
        }
    }
